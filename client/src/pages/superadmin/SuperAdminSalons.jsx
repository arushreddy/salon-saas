...
const PLAN_CFG = {
  ...
  plan3:{ label:'Franchise', color:C.purple, bg:C.purplePale, border:'rgba(167,139,250,0.25)' },
};
...
const getSubStatus = (expiry) => {
  ...
  return         { label:`${Math.ceil(diff/30)}mo`, color:C.teal,  icon:CalendarCheck};
};
...
const PLAN_PRICES = { plan1: 999, plan2: 1999, plan3: 4999 };

/* ─── Discount Section ───────────────────────────────────────────────────── */
// value = { type: 'none'|'percent'|'amount', value: '' }
const DiscountSection = ({ originalAmount, value, onChange }) => {
  ...
  const discountAmt = (() => {
    if (type === 'percent') {
      ...
      return Math.round((originalAmount * pct) / 100);
    }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <label style={{ fontSize:11, fontWeight:600, color:C.inkMuted, textTransform:'uppercase', letterSpacing:'0.05em' }}>
          Discount
        </label>
        {hasDiscount && (
          <span style={{ fontSize:11, fontWeight:700, color:C.green }}>
            -{fmtMoney(discountAmt)} off
          </span>
        )}
      </div>
      ...
      <div style={{ display:'flex', gap:6 }}>
        {[['none','No Discount'],['percent','% Off'],['amount','₹ Off']].map(([v,l]) => (
          <button key={v} type="button" onClick={() => set('type', v)} style={{
            ...
            background: type===v ? C.purplePale : C.bgCard2,
            border: `1px solid ${type===v ? C.purple+'50' : C.border}`,
            color: type===v ? C.purple : C.inkMuted,
          }}>{l}</button>
        ))}
      </div>
      ...
      {type !== 'none' && (
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ position:'relative', flex:1 }}>
            <span style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', fontSize:13, color:C.inkMuted, pointerEvents:'none' }}>
              {type === 'percent' ? '%' : '₹'}
            </span>
            <input
              type="number" min="0"
              max={type === 'percent' ? 100 : originalAmount}
              value={discVal}
          ...
          {type === 'percent' && originalAmount > 0 && (
            <span style={{ fontSize:12, color:C.inkMuted, whiteSpace:'nowrap' }}>
      ...
      {originalAmount > 0 && (
        <div style={{
          background: C.bgCard2, borderRadius:10, padding:'12px 14px',
          border: `1px solid ${hasDiscount ? C.green+'30' : C.border}`,
        }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:C.inkMuted, marginBottom: hasDiscount ? 6 : 0 }}>
            <span>Original amount</span>
            <span>{fmtMoney(originalAmount)}</span>
          </div>
          {hasDiscount && (
            <>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:C.green, marginBottom:6 }}>
                <span>Discount {type === 'percent' ? `(${discVal}%)` : ''}</span>
...
const Btn = ({children,onClick,disabled,variant='primary',small,style:sx={},...p}) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} {...p} style={{display:'flex',alignItems:'center',gap:6,padding:small?'6px 12px':'9px 16px',borderRadius:9,fontSize:small?12:13,fontWeight:600,cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.6:1,...v[variant],...sx}}>
      {children}
    </button>
  );
};
...
const CreateModal = ({onClose,onSaved}) => {
  ...
  return (
    <Modal title="Create New Salon" icon={Plus} onClose={onClose} maxWidth={600}>
      <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:6}}>
          ...
          <div style={{marginTop:14}}><DurationPicker value={duration} onChange={setDuration}/></div>
        </div>
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:6}}>
          <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>💰 Initial Payment (optional)</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <Inp label="Amount (₹)" type="number" min="0" value={payment.amount} onChange={e=>setPayment(p=>({...p,amount:e.target.value}))} placeholder="e.g. 1999"/>
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                {[{value:'cash',label:'💵 Cash'},{value:'upi',label:'📱 UPI'},{value:'card',label:'💳 Card'},{value:'bank_transfer',label:'🏦 Bank'},{value:'other',label:'🔗 Other'}].map(m=>(
...
const EditModal = ({salon,onClose,onSaved}) => {
  ...
  return (
    <Modal title={`Edit — ${salon.name}`} icon={Edit3} onClose={onClose} maxWidth={580}>
      <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
        {err&&<div style={{background:C.redPale,border:`1px solid ${C.redBorder}`,borderRadius:9,padding:'10px 14px',fontSize:13,color:C.red}}>{err}</div>}
        <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:'0.1em'}}>Salon Details</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:6}}>
          <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>Features Override</div>
...
/* ─── Renew Modal ────────────────────────────────────────────────────────── */
const PAYMENT_METHODS=[{value:'cash',label:'💵 Cash'},{value:'upi',label:'📱 UPI'},{value:'card',label:'💳 Card'},{value:'bank_transfer',label:'🏦 Bank Transfer'},{value:'cheque',label:'📄 Cheque'},{value:'other',label:'🔗 Other'}];
...
const RenewModal=({salon,onClose,onSaved})=>{
  ...
  // Compute discount and final amount
  ...
  const loadHistory=useCallback(async()=>{
    setHistLoading(true);
    try{const{data}=await api.get(`/superadmin/salons/${salon._id}/payments`);setHistory(data.payments||[]);setTotalPaid(data.totalRevenue||0);}
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      if(originalAmt>0){
        body.amount=finalAmt;           // store the discounted amount
        body.originalAmount=originalAmt;
        body.discountAmount=discountAmt;
        body.method=method;
      ...
      setSuccess(true);setTimeout(()=>{onSaved?.();onClose();},1200);
    }catch(e){setErr(e.response?.data?.message||'Failed');}finally{setSaving(false);}
  };
  ...
  return (
    <Modal title="Renew Subscription" icon={RotateCcw} iconBg={C.tealPale} iconColor={C.teal} onClose={onClose} maxWidth={560}>
      <div style={{display:'flex',borderBottom:`1px solid ${C.border}`,padding:'0 22px'}}>
        {[['renew','Renew'],['history','Payment History']].map(([v,l])=>(
          <button key={v} onClick={()=>setTab(v)} style={{padding:'10px 16px',background:'none',border:'none',borderBottom:tab===v?`2px solid ${C.teal}`:'2px solid transparent',color:tab===v?C.teal:C.inkMuted,fontSize:13,fontWeight:tab===v?700:400,cursor:'pointer',marginBottom:-1}}>{l}</button>
        ))}
      </div>
      {tab==='renew'&&(
        <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={{background:C.bgCard2,borderRadius:11,padding:'12px 14px'}}>
            <div style={{fontSize:13,fontWeight:600,color:C.ink}}>{salon.name}</div>
            <div style={{display:'flex',gap:16,marginTop:5,fontSize:12,color:C.inkMuted}}>
              <span>Plan: <b style={{color:C.gold}}>{salon.plan}</b></span>
              {salon.subscriptionExpiry&&<span>Expires: <b style={{color:C.red}}>{fmtDate(salon.subscriptionExpiry)}</b></span>}
            </div>
          </div>
          {err&&<div style={{background:C.redPale,border:`1px solid ${C.redBorder}`,borderRadius:9,padding:'10px 14px',fontSize:13,color:C.red}}>{err}</div>}
          {success
            ?<div style={{background:C.greenPale,border:`1px solid ${C.greenBorder}`,borderRadius:12,padding:'20px',textAlign:'center'}}>
               <Check size={22} color={C.green} style={{margin:'0 auto 8px',display:'block'}}/>
               <div style={{fontWeight:700,color:C.green,fontSize:14}}>Subscription renewed!</div>
             </div>
            :<>
               <Sel label="Plan" value={plan} onChange={e=>setPlan(e.target.value)}>
                 <option value="plan1">Basic</option>
                 <option value="plan2">Online Booking</option>
                 <option value="plan3">Franchise</option>
               </Sel>
               ...
               <div style={{borderTop:`1px solid ${C.border}`,paddingTop:6}}>
                 <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>💰 Payment Received</div>
                 <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                   <Inp label="Original Amount (₹)" type="number" min="0" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="e.g. 1999"/>
                   <div style={{display:'flex',flexDirection:'column',gap:5}}>
                     <label style={{fontSize:11,fontWeight:600,color:C.inkMuted,textTransform:'uppercase',letterSpacing:'0.05em'}}>Payment Mode</label>
                     <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
      ...
      {tab==='history'&&(
        <div style={{padding:'16px 22px',display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:C.goldPale,border:`1px solid ${C.gold}30`,borderRadius:12,padding:'14px 18px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <span style={{fontSize:12,color:C.inkMuted}}>Total collected</span>
            <span style={{fontSize:22,fontWeight:800,color:C.gold}}>{fmtMoney(totalPaid)}</span>
          </div>
          {histLoading
            ?<div style={{padding:'28px',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}><RefreshCw size={16} color={C.gold} style={{animation:'spin 1s linear infinite'}}/><span style={{color:C.inkMuted,fontSize:13}}>Loading…</span></div>
            :history.length===0
              ...
              :<div style={{display:'flex',flexDirection:'column',maxHeight:380,overflowY:'auto'}}>
                {history.map((p,i)=>(
                  <div key={p._id} style={{padding:'12px 0',display:'grid',gridTemplateColumns:'1fr auto',gap:12,alignItems:'start',borderBottom:i<history.length-1?`1px solid ${C.border}`:'none'}}>
                    <div>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                        <span style={{fontSize:15}}>{MI[p.method]||'🔗'}</span>
                        <span style={{fontSize:13,fontWeight:600,color:C.ink}}>{PAYMENT_METHODS.find(m=>m.value===p.method)?.label.replace(/^.+ /,'')||p.method}</span>
                        <span style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:5,background:C.purplePale,color:C.purple}}>{p.plan}</span>
                      </div>
                      <div style={{fontSize:11,color:C.inkMuted}}>
                        {fmtDate(p.createdAt)}
                        {p.transactionId&&<span> · Ref: <b style={{color:C.inkMid}}>{p.transactionId}</b></span>}
                        {p.durationMonths&&<span> · {p.durationMonths}mo</span>}
                        {p.periodEnd&&<span> · till {fmtDate(p.periodEnd)}</span>}
                        {p.discountAmount>0&&<span style={{color:C.green}}> · {fmtMoney(p.discountAmount)} discount applied</span>}
                      </div>
                      {p.collectedBy?.name&&<div style={{fontSize:11,color:C.inkMuted,marginTop:2}}>By: {p.collectedBy.name}</div>}
                    </div>
                    <div style={{fontSize:16,fontWeight:800,color:C.green}}>{fmtMoney(p.amount)}</div>
                  </div>
                ))}
              </div>
          }
    ...
    <Modal title={`Suspend "${salon.name}"`} icon={Slash} iconBg={C.redPale} iconColor={C.red} onClose={onClose}>
      <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:5}}>
          ...
          <textarea value={reason} onChange={e=>setReason(e.target.value)} rows={3} placeholder="Non-payment, policy violation…" style={{padding:'9px 12px',borderRadius:9,fontSize:13,border:`1px solid ${C.border}`,background:C.bgCard2,color:C.ink,resize:'vertical',outline:'none',fontFamily:'inherit'}} onFocus={e=>e.target.style.borderColor=C.gold} onBlur={e=>e.target.style.borderColor=C.border}/>
        </div>
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
          <Btn variant="ghost" onClick={onClose}>Cancel</Btn>
...
const ResetPasswordModal=({salon,onClose})=>{
  const [pass,setPass]=useState('');const [saving,setSaving]=useState(false);const [done,setDone]=useState(false);const [err,setErr]=useState('');
...
/* ─── Action Menu — centered overlay, never clipped ─────────────────────── */
const ActionMenu=({salon,onRefresh,onEdit,onSuspend,onRenew,onResetPass})=>{
  ...
  const handleImpersonate=async()=>{
    ...
    try{
      ...
      localStorage.setItem('user',JSON.stringify({name:data.adminUser.name,email:data.adminUser.email,role:'admin',impersonating:salon.name}));
  ...
  const ITEMS=[
    {icon:Edit3,      label:'Edit Salon',          action:()=>{setOpen(false);onEdit();},      color:C.inkMid,  bg:'transparent'},
    {icon:RotateCcw,  label:'Renew Subscription',  action:()=>{setOpen(false);onRenew();},     color:C.teal,    bg:'transparent'},
    {icon:Key,        label:'Reset Password',       action:()=>{setOpen(false);onResetPass();}, color:C.amber,   bg:'transparent'},
    {icon:LogIn,      label:'Login as Admin',       action:handleImpersonate,                   color:C.purple,  bg:'transparent'},
    salon.isSuspended
      ?{icon:ShieldCheck,label:'Unsuspend',         action:handleUnsuspend,                     color:C.green,   bg:'transparent'}
      :{icon:Slash,       label:'Suspend Salon',    action:()=>{setOpen(false);onSuspend();},   color:C.red,     bg:'transparent'},
    {icon:Trash2,label:'Deactivate Salon',          action:handleDelete,                        color:C.red,     bg:'transparent', divider:true},
  ];
        ...
        >
          ...
          >
              ...
              {ITEMS.map((item,i)=>(
                <div key={i}>
                  ...
                  <button onClick={item.action} style={{
                    ...
                    padding:'11px 14px',borderRadius:10,background:'none',
                    border:'none',cursor:'pointer',color:item.color,
                    fontSize:13,fontWeight:500,textAlign:'left',transition:'background 0.1s',
                  }}
            ...
            <div style={{padding:'8px',borderTop:`1px solid ${C.border}`}}>
              <button onClick={()=>setOpen(false)} style={{width:'100%',padding:'10px',borderRadius:10,background:C.bgCard2,border:`1px solid ${C.border}`,color:C.inkMuted,fontSize:13,fontWeight:600,cursor:'pointer'}}>
                Cancel
              </button>
            </div>
          </motion.div>
...
export default function SuperAdminSalons(){
  ...
  const handleSearchChange=(v)=>{clearTimeout(searchTimer.current);searchTimer.current=setTimeout(()=>{setSearch(v);setPage(1);},350);};
  ...
  return(
    <div style={{padding:'24px',maxWidth:1380,margin:'0 auto'}}>
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontFamily:'Cormorant Garamond, Georgia, serif',fontSize:'clamp(20px,3vw,27px)',fontWeight:300,fontStyle:'italic',color:C.ink,margin:0}}>Salon Management</h1>
      ...
      <div style={{background:C.bgCard,borderRadius:14,border:`1px solid ${C.border}`,padding:'14px 16px',marginBottom:16,display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
        <div style={{display:'flex',alignItems:'center',gap:8,background:C.bgCard2,border:`1px solid ${C.border}`,borderRadius:10,padding:'0 12px',height:38,flex:'1 1 220px',maxWidth:320}}>
          ...
          <input placeholder="Search salons, email, slug…" onChange={e=>handleSearchChange(e.target.value)} style={{flex:1,background:'none',border:'none',outline:'none',fontSize:13,color:C.ink}}/>
        </div>
        <select value={planFilter} onChange={e=>{setPlanFilter(e.target.value);setPage(1);}} style={{padding:'0 12px',height:38,borderRadius:10,fontSize:13,border:`1px solid ${C.border}`,background:C.bgCard2,color:planFilter?C.ink:C.inkMuted}}>
          <option value="">All Plans</option><option value="plan1">Basic</option><option value="plan2">Online Booking</option><option value="plan3">Franchise</option>
        </select>
        <select value={statusFilter} onChange={e=>{setStatusFilter(e.target.value);setPage(1);}} style={{padding:'0 12px',height:38,borderRadius:10,fontSize:13,border:`1px solid ${C.border}`,background:C.bgCard2,color:statusFilter?C.ink:C.inkMuted}}>
          <option value="">All Status</option><option value="active">Active</option><option value="suspended">Suspended</option><option value="inactive">Inactive</option><option value="expired">Expired</option>
        </select>
        {(planFilter||statusFilter||search)&&(
          <button onClick={()=>{setPlanFilter('');setStatusFilter('');setSearch('');setPage(1);}} style={{background:'none',border:`1px solid ${C.border}`,borderRadius:9,padding:'7px 12px',cursor:'pointer',color:C.inkMuted,display:'flex',alignItems:'center',gap:6,fontSize:12}}>
            <X size={12}/> Clear
          </button>
        )}
      ...
      <div style={{background:C.bgCard,borderRadius:16,border:`1px solid ${C.border}`,overflow:'hidden'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 160px 110px 130px 130px 100px 52px',padding:'10px 16px',borderBottom:`1px solid ${C.border}`,background:C.bgCard2}}>
        ...
        {loading
          ...
          :salons.length===0
            ?<div style={{padding:'48px',textAlign:'center'}}>
               <Building2 size={32} color={C.border} style={{margin:'0 auto 12px',display:'block'}}/><div style={{color:C.inkMuted,fontSize:14}}>No salons found</div>
             </div>
            :salons.map((salon,i)=>{
               ...
               return(
                 <div key={salon._id} style={{display:'grid',gridTemplateColumns:'1fr 160px 110px 130px 130px 100px 52px',padding:'13px 16px',alignItems:'center',borderBottom:`1px solid ${C.border}`,transition:'background 0.1s'}}
                 ...
                 >
                   ...
                   <div style={{paddingRight:12}}><Badge color={plan.color} bg={plan.bg} border={plan.border}>{plan.label}</Badge></div>
                   <div style={{paddingRight:12}}>
                     <div style={{display:'flex',alignItems:'center',gap:5}}>
                       <SubIcon size={12} color={sub.color}/><span style={{fontSize:12,color:sub.color,fontWeight:500}}>{sub.label}</span>
                     </div>
                   ...
                   <ActionMenu salon={salon} onRefresh={load} onEdit={()=>setEditTarget(salon)} onSuspend={()=>setSuspendTarget(salon)} onRenew={()=>setRenewTarget(salon)} onResetPass={()=>setResetPassTarget(salon)}/>
                 </div>
               );
             })
        }
      ...
      {/* Modals */}
      <AnimatePresence>{showCreate&&<CreateModal key="m-create" onClose={()=>setShowCreate(false)} onSaved={load}/>}</AnimatePresence>
      <AnimatePresence>{editTarget&&<EditModal key="m-edit" salon={editTarget} onClose={()=>setEditTarget(null)} onSaved={load}/>}</AnimatePresence>
      <AnimatePresence>{suspendTarget&&<SuspendModal key="m-suspend" salon={suspendTarget} onClose={()=>setSuspendTarget(null)} onDone={load}/>}</AnimatePresence>
      <AnimatePresence>{renewTarget&&<RenewModal key="m-renew" salon={renewTarget} onClose={()=>setRenewTarget(null)} onSaved={load}/>}</AnimatePresence>
      <AnimatePresence>{resetPassTarget&&<ResetPasswordModal key="m-reset" salon={resetPassTarget} onClose={()=>setResetPassTarget(null)}/>}</AnimatePresence>
