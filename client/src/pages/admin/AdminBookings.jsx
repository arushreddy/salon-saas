...
import { Loader2, Calendar, Filter, Plus, UserPlus, Sparkles } from 'lucide-react';
...
const AdminBookings = () => {
  ...
  const [walkInForm, setWalkInForm] = useState({ customerName: '', customerPhone: '', serviceId: '', paymentMethod: 'cash', notes: '' });
  ...
  const handleStatusChange = async (bookingId, status, paymentMethod, paymentStatus) => {
    try {
      ...
      // If completing with payment, create payment record
  ...
  const handleWalkIn = async (e) => {
    ...
    try {
      ...
      await api.post('/bookings/walk-in', walkInForm);
      setShowWalkIn(false);
      setWalkInForm({ customerName: '', customerPhone: '', serviceId: '', paymentMethod: 'cash', notes: '' });
      fetchBookings();
      fetchStats();
    } catch (err) {
      console.error(err);
    ...
    <div>
      ...
      {showWalkIn && (
        <motion.div
          ...
          className="bg-white rounded-2xl border border-gold/10 p-5 mb-6"
        >
          ...
          <form onSubmit={handleWalkIn} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <button
              ...
              className="luxury-gradient text-white py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-40"
            >
