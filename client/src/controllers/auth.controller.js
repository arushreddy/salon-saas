...
const getMe = async (req, res, next) => {
  try {
    ...
    // Attach salon plan + slug so frontend can show website link + gate features
    if (user.salonId) {
      const salon = await Salon.findById(user.salonId)
        .select("plan slug subscriptionExpiry features name")
        .lean();
      if (salon) {
        safeUser.plan                = salon.plan;
        safeUser.salonSlug           = salon.slug;
        safeUser.salonName           = salon.name;
        safeUser.features            = salon.features;
        safeUser.subscriptionExpiry  = salon.subscriptionExpiry;
      }
    }
